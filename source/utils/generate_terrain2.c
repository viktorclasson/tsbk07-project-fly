	...
	for(n_x=0; n_x < mapvertexsize;n_x++)
		for(n_z=0;n_z < mapvertexsize;n_z++)
		{
			...
			if((x >= 0) && (x < tex->width) && (z >= 0) && (z < tex->width))
			{
				...
				vertexArray[(n_x + n_z * mapvertexsize)*3 + 1] = tex->imageData[(x + z * tex->width) * (tex->bpp/8)] / heightfactor;
				//clock_gettime(CLOCK_REALTIME, &t2);
			}
			else
			{
			*/
				//vertexArray[(n_x + n_z * mapvertexsize)*3 + 1] = tex->imageData[(n_x + n_z * tex->width) * (tex->bpp/8)] / 30.0;
				//clock_gettime(CLOCK_REALTIME, &t3);
				//int bignumber = 100000000;
				//srand(t3.tv_nsec);
				//float randomvalue = rand()%bignumber;
				// vertexArray[(n_x + n_z * mapvertexsize)*3 + 1] = randomvalue/bignumber;
				//clock_gettime(CLOCK_REALTIME, &t4);
				//printf("RAND: %f \n",randomvalue/bignumber);
			//}
			vertexArray[(n_x + n_z * mapvertexsize)*3 + 1] = 0;
			//Heightmap[n_x][n_z]=vertexArray[(n_x + n_z * mapvertexsize)*3 + 1];
