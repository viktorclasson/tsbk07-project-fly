...
//{
  ...
  static void ParseOBJArray(MeshArrayPtr theMeshArray)
  {
    int tokenType;
    
    bool readFaceFlag = false;
    ...
    while (tokenType != kEOF)
    {
      ...
      {
	case vToken:
	  if(readFaceFlag == true)	//Faces have already been read, this is a new object
	  {
	    if(theMeshArray->meshes[meshIndex]->vertices == NULL)	//Allocate arrays if it they don't exist(first run)
	    {
	      if (vertCount > 0)
		theMeshArray->meshes[meshIndex]->vertices = malloc(sizeof(GLfloat) * vertCount);
	      if (texCount > 0)
		theMeshArray->meshes[meshIndex]->textureCoords = malloc(sizeof(GLfloat) * texCount);
	      if (normalsCount > 0)
		theMeshArray->meshes[meshIndex]->vertexNormals = malloc(sizeof(GLfloat) * normalsCount);
	      if (hasPositionIndices)
		theMeshArray->meshes[meshIndex]->coordIndex = malloc(sizeof(int) * coordCount);
	      if (hasNormalIndices)
		theMeshArray->meshes[meshIndex]->normalsIndex = malloc(sizeof(int) * coordCount);
	      if (hasTexCoordIndices)
		theMeshArray->meshes[meshIndex]->textureIndex = malloc(sizeof(int) * coordCount);
	    }
	    else								//Second run
	    {
	      theMeshArray->meshes[meshIndex]->vertexCount = vertCount/3;
	      theMeshArray->meshes[meshIndex]->coordCount = coordCount;
	      
	      // Counters for tex and normals, texCount and normalsCount
	      theMeshArray->meshes[meshIndex]->texCount = texCount/2;
	      theMeshArray->meshes[meshIndex]->normalsCount = normalsCount/3; // Should be the same as vertexCount!
	      // This assumption could make handling of some models break!
	    }
	    hasPositionIndices = true;
	    hasTexCoordIndices = false;
	  ...
	  ReadOneVertex(theMeshArray->meshes[meshIndex]);
	  break;
	case vnToken:
	  ReadOneNormal(theMeshArray->meshes[meshIndex]);
	  break;
	case vtToken:
	  ReadOneTexture(theMeshArray->meshes[meshIndex]);
	  break;
	case fToken:
	  ReadOneFace(theMeshArray->meshes[meshIndex]);
	  readFaceFlag = true;
	  break;
	case kReal:
	  // Ignore
  ...
  static int ObjectCounter(MeshPtr theMesh)
  {
    int tokenType;
    
    bool readFaceFlag = true;
    int objectCount = 0;
  ...
  static struct MeshArray * LoadOBJArray(const char *filename)
  {
    MeshArray *theMeshArray;
    Mesh *theMesh;
    
    theMeshArray = malloc(sizeof(MeshArray));
    ...
    theMeshArray->meshCount = ObjectCounter(theMesh);
    fclose(fp);
    
    
    
    
    for(int i = 0; i < theMeshArray->meshCount; i++)
    {
      theMeshArray->meshes[i] = malloc(sizeof(Mesh));
      theMeshArray->meshes[i]->coordIndex = NULL;
      theMeshArray->meshes[i]->vertices = NULL;
      // ProcessMesh may deal with these
      theMeshArray->meshes[i]->triangleCountList = NULL;
      theMeshArray->meshes[i]->vertexNormals = NULL;
      theMeshArray->meshes[i]->vertexToTriangleTable = NULL;
      theMeshArray->meshes[i]->textureCoords = NULL;
      theMeshArray->meshes[i]->textureIndex = NULL;
      theMeshArray->meshes[i]->normalsIndex = NULL;
    }
    ...
    ParseOBJArray(theMeshArray);
    fclose(fp);
    
    
    //Allocate the last object in the array
    if (vertCount > 0)
      theMeshArray->meshes[theMeshArray->meshCount-1]->vertices = malloc(sizeof(GLfloat) * vertCount);
    if (texCount > 0)
      theMeshArray->meshes[theMeshArray->meshCount-1]->textureCoords = malloc(sizeof(GLfloat) * texCount);
    if (normalsCount > 0)
      theMeshArray->meshes[theMeshArray->meshCount-1]->vertexNormals = malloc(sizeof(GLfloat) * normalsCount);
    if (hasPositionIndices)
      theMeshArray->meshes[theMeshArray->meshCount-1]->coordIndex = malloc(sizeof(int) * coordCount);
    if (hasNormalIndices)
      theMeshArray->meshes[theMeshArray->meshCount-1]->normalsIndex = malloc(sizeof(int) * coordCount);
    if (hasTexCoordIndices)
      theMeshArray->meshes[theMeshArray->meshCount-1]->textureIndex = malloc(sizeof(int) * coordCount);
    ...
    //Counters for the last mesh in the array
    theMeshArray->meshes[theMeshArray->meshCount-1]->vertexCount = vertCount/3;
    theMeshArray->meshes[theMeshArray->meshCount-1]->coordCount = coordCount;
    
    // Counters for tex and normals, texCount and normalsCount
    theMeshArray->meshes[theMeshArray->meshCount-1]->texCount = texCount/2;
    theMeshArray->meshes[theMeshArray->meshCount-1]->normalsCount = normalsCount/3; // Should be the same as vertexCount!
    // This assumption could make handling of some models break!
  ...
  ModelArray* LoadModelArray(char* name)
  {
    MeshArray* meshes = LoadOBJArray(name);
    ModelArray* models;
    
    models->models = malloc(sizeof(Model) * meshes->meshCount);
    models->modelCount = meshes->meshCount;
    ...
    {
      DecomposeToTriangles(meshes->meshes[i]);
      
      generateNormals(meshes->meshes[i]);
      
      models->models[i] = generateModel(meshes->meshes[i]);
    }
  ...
  void BuildModelVAO2(Model *m/*, GLuint program, char* vertexVariableName, char* normalVariableName, char* texCoordVariableName*/)
  {
  ...
  Model* LoadModelPlus(char* name/*, GLuint program, char* vertexVariableName, char* normalVariableName, char* texCoordVariableName*/)
  {
    Model *m;
  ...
  ModelArray* LoadModelPlusArray(char* name)
  {
    ModelArray *m;
    
    m = LoadModelArray(name);
    
    for(int i = 0; i < m->modelCount; i++)
      BuildModelVAO2(m->models[i]);
    
    return m;
  }
  
  // Loader for inline data to Model (almost same as LoadModelPlus)
  Model* LoadDataToModel(GLfloat *vertices, GLfloat *normals, GLfloat *texCoords, GLfloat *colors, GLuint *indices, int numVert, int numInd)
  {
