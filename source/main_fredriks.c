...
GLfloat plane_yaw = 0;
GLfloat plane_pitch = 0;
GLfloat plane_roll = 0;

Point3D plane_forward;
Point3D plane_up;
...
void draw_object()
{
	// Program
	// Skapa mtwMatrix
	GLfloat mtwMatrix[16], scaleMatrix[16], rotMatrix[16], placeMatrix[16];

	Rx(animation/10,rotMatrix);
	T(plane_x,plane_y,plane_z,placeMatrix);
	S(0.3,0.3,0.3,scaleMatrix);
	Mult(rotMatrix,scaleMatrix,mtwMatrix);
